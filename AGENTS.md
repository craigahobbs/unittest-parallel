...
- `unittest` tests. Golden stderr is compared via `TestMain.assert_output`, which normalizes timing (`<SEC>s`) and `File "...", line N`. It only strips pure-caret lines (`^`); it does **not** strip 3.13+ traceback emphasis (`~~~~~~~~~^^`). Copy existing `sys.version_info` splits in `test_main.py`: keep a `< (3, 11)` arm only to match neighbors (dead on supported 3.11–3.15); add a `< (3, 13)` arm whenever the golden includes an assertion traceback. Each arm is `# pragma: no cover`. The 3.11 split is unittest description strings (`FailureTestCase` vs `FailureTestCase.mock_2`); the 3.13 split is traceback carets.
